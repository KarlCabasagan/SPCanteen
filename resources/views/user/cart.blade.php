@extends('layouts.user')

@section('content')
<div class="container">
    <div class="cart-content">
        <div class="cart-header">
            <a href="/">
                <iconify-icon id="back-btn" icon="material-symbols:arrow-back-ios"></iconify-icon>
            </a>
            <h1 id="mycart-txt">MY CART</h1>
            <a class="close-btn" href="/">
                <iconify-icon id="close-btn" icon="material-symbols-light:close"></iconify-icon>
            </a>
        </div>
        <div class="cart">
            <div class="cart-info">
                <div class="cart-container">
                    <div class="cart-image">
                        <img id="cart-image" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTV1mn8AmFrmldZhG7Lc_uTy-NbSemRXlv0FwYOpQY-Hg&s" alt="">
                    </div>
                    <div class="content-details">
                        <div class="cart-details">
                            <h1 id="cart-name">Chicken Burger</h1>
                            <span id="cart-time">40mins</span>
                        </div>
                        <div class="content-button">
                            <div class="cart-price">
                                <h3 id="cart-price">₱50</h3>
                            </div>
                            <div class="quantity-button">
                                <div class="plus-icon">
                                    <iconify-icon id="quantity-icons" icon="mdi:plus"></iconify-icon>
                                </div>
                                <span>1</span>
                                <div class="minus-icon">
                                    <iconify-icon id="quantity-icons" icon="mdi:minus"></iconify-icon>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="cart-delete">
                        <iconify-icon icon="ion:trash-sharp"></iconify-icon>
                    </div>
                </div>
            <div class="cart-containers">
                <div class="cart-image">
                    <img id="cart-image" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTV1mn8AmFrmldZhG7Lc_uTy-NbSemRXlv0FwYOpQY-Hg&s" alt="">
                </div>
                <div class="content-details">
                    <div class="cart-details">
                        <h1 id="cart-name">Chicken Burger</h1>
                        <span id="cart-time">40mins</span>
                    </div>
                    <div class="content-button">
                        <div class="cart-price">
                            <h3 id="cart-price">₱50</h3>
                        </div>
                        <div class="quantity-button">
                            <div class="plus-icon">
                                <iconify-icon id="quantity-icons" icon="mdi:plus"></iconify-icon>
                            </div>
                            <span>1</span>
                            <div class="minus-icon">
                                <iconify-icon id="quantity-icons" icon="mdi:minus"></iconify-icon>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="cart-delete">
                    <iconify-icon icon="ion:trash-sharp"></iconify-icon>
                </div>
            </div>
            <div class="cart-containers">
                <div class="cart-image">
                    <img id="cart-image" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTV1mn8AmFrmldZhG7Lc_uTy-NbSemRXlv0FwYOpQY-Hg&s" alt="">
                </div>
                <div class="content-details">
                    <div class="cart-details">
                        <h1 id="cart-name">Chicken Burger</h1>
                        <span id="cart-time">40mins</span>
                    </div>
                    <div class="content-button">
                        <div class="cart-price">
                            <h3 id="cart-price">₱50</h3>
                        </div>
                        <div class="quantity-button">
                            <div class="plus-icon">
                                <iconify-icon id="quantity-icons" icon="mdi:plus"></iconify-icon>
                            </div>
                            <span>1</span>
                            <div class="minus-icon">
                                <iconify-icon id="quantity-icons" icon="mdi:minus"></iconify-icon>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="cart-delete">
                    <iconify-icon icon="ion:trash-sharp"></iconify-icon>
                </div>
            </div>
            <div class="cart-containers">
                <div class="cart-image">
                    <img id="cart-image" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTV1mn8AmFrmldZhG7Lc_uTy-NbSemRXlv0FwYOpQY-Hg&s" alt="">
                </div>
                <div class="content-details">
                    <div class="cart-details">
                        <h1 id="cart-name">Chicken Burger</h1>
                        <span id="cart-time">40mins</span>
                    </div>
                    <div class="content-button">
                        <div class="cart-price">
                            <h3 id="cart-price">₱50</h3>
                        </div>
                        <div class="quantity-button">
                            <div class="plus-icon">
                                <iconify-icon id="quantity-icons" icon="mdi:plus"></iconify-icon>
                            </div>
                            <span>1</span>
                            <div class="minus-icon">
                                <iconify-icon id="quantity-icons" icon="mdi:minus"></iconify-icon>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="cart-delete">
                    <iconify-icon icon="ion:trash-sharp"></iconify-icon>
                </div>
            </div>
            <div class="cart-containers">
                <div class="cart-image">
                    <img id="cart-image" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTV1mn8AmFrmldZhG7Lc_uTy-NbSemRXlv0FwYOpQY-Hg&s" alt="">
                </div>
                <div class="content-details">
                    <div class="cart-details">
                        <h1 id="cart-name">Chicken Burger</h1>
                        <span id="cart-time">40mins</span>
                    </div>
                    <div class="content-button">
                        <div class="cart-price">
                            <h3 id="cart-price">₱50</h3>
                        </div>
                        <div class="quantity-button">
                            <div class="plus-icon">
                                <iconify-icon id="quantity-icons" icon="mdi:plus"></iconify-icon>
                            </div>
                            <span>1</span>
                            <div class="minus-icon">
                                <iconify-icon id="quantity-icons" icon="mdi:minus"></iconify-icon>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="cart-delete">
                    <iconify-icon icon="ion:trash-sharp"></iconify-icon>
                </div>
            </div>
        </div>
        </div>
        <div class="price-container">
            <div class="price-txt">
                <div class="products-selected">
                    <span>Selected</span>
                    <span>1</span>
                </div>
                <div class="products-total">
                    <span id="total-txt">Total</span>
                    <h2>₱50</h2>
                </div>
            </div>
            <a class="order-btn" href="payment">
                <button class="order-now">
                    <span id="order-txt">Order Now</span>
                </button>
            </a>
        </div>
    </div>
</div>
@endsection